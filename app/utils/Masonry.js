...
const Masonry = ({
  ...
  const handleMouseEnter = (id, element) => {
    ...
    const imgDiv = element.querySelector('.relative');
    if (imgDiv) {
      gsap.to(imgDiv, { filter: 'grayscale(0)', duration: 0.3 });
    }
  ...
  const handleMouseLeave = (id, element) => {
    ...
    // Reaplica grayscale ao sair do hover
    const imgDiv = element.querySelector('.relative');
    if (imgDiv) {
      gsap.to(imgDiv, { filter: 'grayscale(1)', duration: 0.3 });
    }
    if (colorShiftOnHover) {
      const overlay = element.querySelector(".color-overlay");
      ...
      {grid.map((item) => (
        ...
        >
          <div
            ...
            style={{ backgroundImage: `url(${item.img})`, filter: 'grayscale(1)', transition: 'filter 0.3s' }}
          >
            {colorShiftOnHover && (
